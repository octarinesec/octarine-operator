...
func (webhookConfig *validatingWebhookConfigurationV1Beta1) GetWebhooks() []WebhookAdapter {
	result := make([]WebhookAdapter, 0, len(webhookConfig.Webhooks))
	for i := range webhookConfig.Webhooks {
		var webhookAdapter WebhookAdapter = (*validatingWebhookV1Beta1)(&webhookConfig.Webhooks[i])
		result = append(result, webhookAdapter)
	}
	return result
}

func (webhookConfig *validatingWebhookConfigurationV1Beta1) SetWebhooks(webhooks []WebhookAdapter) ([]WebhookAdapter, error) {
