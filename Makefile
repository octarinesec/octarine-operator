...
# cbartifactory/octarine-operator-bundle:$VERSION and cbartifactory/octarine-operator-catalog:$VERSION.
...
# Run with Delve for development purposes against the configured Kubernetes cluster in the KUBE_CONFIG env var
...
.PHONY: install
install: manifests kustomize ## Install CRDs into the K8s cluster specified in KUBECONFIG env var.
	$(KUSTOMIZE) build $(PATH_TO_CRDS) | kubectl apply -f -

.PHONY: uninstall
uninstall: manifests kustomize ## Uninstall CRDs from the K8s cluster specified in KUBECONFIG env var.
	$(KUSTOMIZE) build $(PATH_TO_CRDS) | kubectl delete --ignore-not-found=$(ignore-not-found) -f -

# Generate and bundle all operator components in a single YAML file

.PHONY: create_operator_spec
...
.PHONY: deploy
deploy: create_operator_spec ## Deploy controller to the K8s cluster specified in KUBECONFIG env var.
	- kubectl apply -f operator.yaml
	rm -f operator.yaml

.PHONY: undeploy
undeploy: create_operator_spec ## Undeploy controller from the K8s cluster specified in KUBECONFIG env var. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	- kubectl delete --ignore-not-found=$(ignore-not-found) -f operator.yaml
	rm -f operator.yaml
